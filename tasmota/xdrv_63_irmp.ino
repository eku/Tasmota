/*
  xdrv_63_irmp.ino - complete integration of IRMP for Tasmota
...
#define F_INTERRUPTS 15000 // Instead of default 15000 to support LEGO + RCMM protocols
...
enum Xdrv63_IrErrors { IE_NO_ERROR, IE_INVALID_JSON, IE_SYNTAX_IRSND, IE_UNKNOWN };

static const char Xdrv63_IrSendCommands[] PROGMEM = "|" D_CMND_IRSND ;

static void (* const Xdrv63_IrSendCommand[])(void) PROGMEM = {
  &Xdrv63_IrSendCmnd };
...
void Xdrv63_IrSendInit(void)
{
  AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_IRR "tx pin=%d"), Pin(GPIO_IRSEND));
  irsnd_init(Pin(GPIO_IRSEND)); // an IR led is at GPIO_IRSEND
}

bool Xdrv63_Validate(unsigned long value)
{
  return !((errno == ERANGE && (value == LONG_MAX || value == LONG_MIN)) || (errno != 0 && value == 0));
}

#if IRMP_PROTOCOL_NAMES == 1
unsigned long Xdrv63_ProtocolName2Id(const char *name)
{
...
uint8_t Xdrv63_IrSendIrSendCmndJson(void)
{
  char *data = RemoveSpace(XdrvMailbox.data);
  AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_IRR "irsnd json=%s"), data);
  ...
  {
#if IRMP_PROTOCOL_NAMES == 1
    value = Xdrv63_ProtocolName2Id(var_proto.getStr());
#else
    return IE_SYNTAX_IRSND;
#endif
  }
  if (Xdrv63_Validate(value) == false || value == 0 || value > IRMP_N_PROTOCOLS)
    return IE_SYNTAX_IRSND;
  ...
  if (Xdrv63_Validate(value) == false || value > USHRT_MAX)
    return IE_SYNTAX_IRSND;
  ...
  if (Xdrv63_Validate(value) == false || value > UCHAR_MAX)
    return IE_SYNTAX_IRSND;
...
void Xdrv63_IrSendCmndResponse(uint32_t error)
{
  switch (error)
  {
...
void Xdrv63_IrSendCmnd(void)
{
  uint8_t error = IE_SYNTAX_IRSND;

  if (XdrvMailbox.data_len)
  {
    error = Xdrv63_IrSendIrSendCmndJson();
  }
  Xdrv63_IrSendCmndResponse(error);
}
...
void Xdrv63_IrReceiveInit(void)
{
  AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_IRR "rx pin=%d"), Pin(GPIO_IRRECV));
  irmp_init(Pin(GPIO_IRRECV));
}

void Xdrv63_IrReceiveCheck(void)
{
  IRMP_DATA irmp_data;
  ...
  if (PinUsed(GPIO_IRSEND) || PinUsed(GPIO_IRRECV))
  {
    switch (function)
    {
      case FUNC_PRE_INIT:
        ...
        {
          Xdrv63_IrReceiveInit();
        }
        break;
      case FUNC_EVERY_50_MSECOND:
        if (PinUsed(GPIO_IRRECV))
        {
          Xdrv63_IrReceiveCheck();  // check if there's anything on IR side
        }
        break;
      case FUNC_COMMAND:
        if (PinUsed(GPIO_IRSEND))
        {
          result = DecodeCommand(Xdrv63_IrSendCommands, Xdrv63_IrSendCommand);
        }
        break;
    }
  }
